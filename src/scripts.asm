...
; - DE: pointer to action parameters (will quickly switch to HL for compatibility with the Speccy version)
...
run_script:
	...
	ex de, hl	; HL: pointer, DE: parameters
...
; Get the enemy ROM page:
; INPUT:
;	- IX: pointer to enemy entity
get_enemy_rom_page:
	ld a, (ix+10)
	rrca
	rrca
	...
	add a, OBJECT_ENEMY_SKELETON ; make it base 0
	call GetSpriteROMPage	; sprite rom page in A
...
action_joystick:
	...
	ex de, hl		; now HL points to the address
action_joystick_call:
	call IndCall	; and jump to the function
action_player:
	xor a			; return 0 as offset for next script
	ret
...
script_player:
	...
	bit 5, a			; BIT 5 is FIRE 2 (ACTION)
...
; No params
action_kill_player:
	ex de, hl ; Speccy version compatibility (not needed this time, but hey)
 	ld a, (ix+5)
	add a, a
	ld l, a
...
action_remove_boxes_finished:
    ...
    push ix
	cp OBJECT_BOX_LEFT
	jp z, action_remove_boxes_finished_left
