...
initplayerdata:
	dw	SPRITE_OFFSET_IDLE			;barbaro_idle		;animation position
	db	SPR_24x32_MIRROR | BARBARIAN_ROM_PAGE		;sprite type & ROM Bank
	db	0			;X
	db	24			;Y
	db	3			;3 chars in X
	db	4			;4 chars in Y
...
get_sprite_pointer_ix:
	; We will always use this function when getting the sprite
    ; pointer for a weapon animation. If this is the barbarian,
    ; set the proper offset for the weapon page
	ld a, ixl
	cp ENTITY_PLAYER_POINTER%256
    jr nz, get_sprite_pointer_ix_offset0 ; this is the barbarian
	ld a, (player_current_weapon)
	and a
	jr z, get_sprite_pointer_ix_offset0
	inc a ; the rom page offset is 1+weapon number
	jr get_sprite_pointer_ix_common
get_sprite_pointer_ix_offset0:
	xor a
get_sprite_pointer_ix_common:
	ld (rom_offset_weapon), a
	ld e, (ix+0)
	ld d, (ix+1)		; DE has the sprite pointer
	ld ixh, d
	ld ixl, e		; Get the sprite pointer in IX
	ret
...
landed_common:
	...
	add a, e			; and divide by 16 to get to stile coordinates
	rra
	rra
	rra
	rra
	...
	ld a, (newx)		; get X position and divide by 16 to get to stile coordinates
	rra
	rra
	rra
	rra
...
entity_fire_checkright:
	bit 3, a
	jr z, entity_fire_checknothing	; FIRE + RIGHT is run right
	...
	call get_sprite_pointer_iy
	jp entity_startrunning
; After some tester's feedback, we're removing FIRE+UP as unsheathe
;entity_fire_checkup:
;	bit 0, a
...
entity_idle_switchon:
	...
	ld (ix+6), 0		; animation is now 0
	call get_sprite_pointer_ix ; Get the sprite pointer in IX
	call setrompage_ix_plus1
	ld hl, SPRITE_OFFSET_SWITCH
	ld (ix+0), l
	ld (ix+1), h		; Store animation position
	ld a, (ix+5)
	and 1				
...
entity_startmove_common:
	ld (ix+6), 0		; animation is now 0
	call get_sprite_pointer_ix ; Get the sprite pointer in IX
	ld a, (sprite_rom_page)
	or c
	ld (ix+2), a		; store sprite type and page
	ld hl, SPRITE_OFFSET_WALK
	jp entity_updatesprite
...
entity_idle_turn_common:
	...
	ld a, (sprite_rom_page)
	or c
	ld (ix+2), a		; store sprite type
	ld hl, SPRITE_OFFSET_TURN
entity_updatesprite:
	ld (ix+0), l
	ld (ix+1), h		; Store animation position
	ld b, (ix+3)
...
entity_startshortjump:
	...
	ld a, (sprite_rom_page)
	inc a
	or c				; FIXME: this is only done by the barbarian, we need to make sure we are using the right page
	ld (ix+2), a		; Store sprite type and page
	ld hl, SPRITE_OFFSET_SHORTJUMP
	jp entity_updatesprite	
...
entity_idle_up_finishedcheck:
	...
	ld (ix+5), a		; set state
	ld (ix+6), 0		; reset animation
	call setrompage_iy_plus1
	ld hl, SPRITE_OFFSET_JUMP_UP
	ld (iy+0), l
	ld (iy+1), h		; Store animation position
...
entity_down_down:
	...
	call get_sprite_pointer_ix; Get the sprite pointer in IX
	call setrompage_ix_plus1
	ld hl, SPRITE_OFFSET_JUMP_UP+384*7
	ld (ix+0), l
	ld (ix+1), h		; Store animation position
	ld b, (ix+3)
	ld c, (ix+4)		; Keep position
...
;entity_walk_sword_checkaction:
;	bit 5, a			; BIT 5 is action
	jp entity_set_idle_sword
;	jp entity_idlesword_down
...
entity_swordhigh_slash_normal:
	; for this animation, we add 384 to the animation position and get a second sprite
	call NewSprite		; get address of new sprite in HL
...
entity_swordhigh_injury_ok_cont:
	; if IX ==  ENTITY_PLAYER_POINTER, then set sprite_rom_page to be the barbarian rom page
	; otherwise, make it the enemy page
	ld a, ixl
	cp ENTITY_PLAYER_POINTER%256
...
injury_with_sword_cont:
	...
	call get_sprite_pointer_ix ; Get the sprite pointer in IX
	call setrompage_ix
	ld hl, SPRITE_OFFSET_OUCH_SWORD
injury_done:
	ld (ix+0), l
	ld (ix+1), h		; Store animation position
	...
	ld a, (hitting_entity_looking_at)
	rrca	; make it the most significant bit
	ld h, a
	ld a, (ix+2)
	and $7f
	or h
...
entity_grab_common:
	...
	ld (newy), a
	call setrompage_iy_plus1
	ld hl, SPRITE_OFFSET_JUMP_UP+384*2
	ld (iy+0), l
	ld (iy+1), h		; Store animation position
	jp player_updatesprite
;	call player_updatesprite
...
entity_secondary:
	...
	ld a, (iy+2)
	inc a
	ld (ix+2), a					; sprite type | sprite page (+1)
	ld b, (iy+3)					; xpos
	ld a, (iy+4)
	sub 32
	ld c, a					; ypos
	ld e, (iy+0)
	ld d, (iy+1)					; sprite address. The sprite for the secondary one is primary + 1 page
	ld (ix+0), e
	ld (ix+1), d					; and place in the sprite
...
setrompage_iy:
	ld a, (iy+2)
	and $80
	ld c, a		; store sprite type in C
	ld a, (sprite_rom_page)
	or c
	ld (iy+2), a	; this is the proper page+sprite type
	ret
...
setrompage_ix:
	...
	or c
	ld c, a
	ld a, (rom_offset_weapon)
	add a, c
	ld (ix+2), a	; this is the proper page+sprite type
	ret
